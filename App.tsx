...
import { auth, db, appId, storage, EmailAuthProvider, Timestamp } from './services/firebase';
...
const App: React.FC = () => {
    ...
    const handleAdminSettingsUpdate = useCallback(async (updatedData: Partial<AdminSettings>) => {
        ...
        try {
            ...
            await settingsDocRef.set(updatedData, { merge: true });
        } catch (error) {
                ...
                profileUnsubscribe = userDocRef.onSnapshot(async (docSnap) => {
                    ...
                    if (docSnap.exists) {
                        ...
                        if (Object.keys(profileUpdate).length > 0) {
                           await docSnap.ref.set(profileUpdate, { merge: true });
                        }
